...
public interface IEntityClaimSystem
{
...
public static class EntityClaimSystemUtils
{
    public static Claim<T> Create<T>(this IEntityClaimSystem system, Func<string, T> create)
        where T : notnull, IIdentifiable
        => system.Create(null, create);

    public static async ValueTask<IEnumerable<Claim<T>>> Load<T>(this IEntityClaimSystem system, IEnumerable<Id<T>> ids)
        where T : notnull, IIdentifiable
        => await system.Load<T>(ids.Select(x => x.Value));

    public static async ValueTask<Claim<T>> Load<T>(this IEntityClaimSystem system, string id)
        where T : notnull, IIdentifiable
        => (await system.Load<T>(Enumerable.Repeat(id, 1))).First();

    public static async ValueTask<Claim<T>> Load<T>(this IEntityClaimSystem system, Id<T> id)
        where T : notnull, IIdentifiable
        => await system.Load<T>(id.Value);

    public static async ValueTask Delete(this IEntityClaimSystem system, string id)
        => await system.Delete(Enumerable.Repeat(id, 1));

    public static void Mark(this IEntityClaimSystem system, string id)
        => system.Mark(Enumerable.Repeat(id, 1));
}

public class EntityClaimSystem : IEntityClaimSystem
{
    public EntityClaimSystem(IIdGenerator idGenerator, IEntityStore store)
    {
